#!/bin/bash
# 发射 hz-sft 到 Vast(付费!)。凭据从本地 aws profile 注入,不写进 YAML。
# 用法:
#   ./sky/launch-hz-sft.sh            # 干跑(默认,不花钱)
#   GPU_BURST_LIVE=1 ./sky/launch-hz-sft.sh --confirm-paid   # 真发射
set -euo pipefail
cd "$(dirname "$0")/.."
